...
const extractDataTypeData = (line, index) => {
  ...
  switch (line[4].slice(0, -1)) {
    ...
    default:
      throw new Error(
        `Can't read dataType ${line[4].slice(
          0,
          -1
        )} at line ${index} in the .dbc file. It should either be 0 (int), 1 (float) or 2 (double). This will cause unfixable incorrect data.`
      );
  }
