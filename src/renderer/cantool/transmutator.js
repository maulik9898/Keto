...
const parseDbc = (dbcString, options = {}) => {
  ...
  const dbcArray = dbcString.split('\n');
  ...
  const dbcData = _.map(dbcArray, (line, index) => {
  ...
  _.each(dbcData, (line, index) => {
    ...
    switch (line[0]) {
      case 'BO_': // BO_ 2147486648 Edgy: 8 Vector__XXX
        ...
        if (line.length !== 5) {
          throw new Error(
            `BO_ on line ${
              index + 1
            } does not follow DBC standard (should have five pieces of text/numbers), all signals in this message won't have a PGN or source.`
          );
        ...
        if (!_.isEmpty(currentBo)) {
          if (_.isEmpty(currentBo.signals)) {
            ...
            problems.push({
              severity: 'warning',
              line: currentBo.lineInDbc,
              description:
                'BO_ does not contain any SG_ lines; message does not have any signals.',
            });
          }
        ...
        if (isNaN(canId)) {
          throw new Error(
            `BO_ CAN ID on line ${
              index + 1
            } is not a number, all signals in this message won't have a PGN or source.`
          );
        ...
        if (duplicateCanId) {
          ...
          problems.push({
            ...
            line: index + 1,
            description:
              'BO_ CAN ID already exists in this file. Nothing will break on our side, but the data will be wrong because the exact same CAN data will be used on two different signals.',
          });
        }

        // Split CAN ID into PGN, source and priority (if isExtendedFrame)
        try {
          const { isExtendedFrame, priority, pgn, source } = splitCanId(canId);
          let label = snakeCase(name);

          if (options.extendedLabel) label = snakeCase(currentBo.name) + label;
          ...
          throw new Error(
            `The parser broke unexpectedly :( Please contact the VT team and send them the DBC file you were trying to parse as well as this error message:\n${e}`
          );
        }
      ...
      case 'SG_': // SG_ soc m0 : 8|8@1+ (0.5,0) [0|100] "%" Vector__XXX
        if (line.length < 8 || line.length > 9) {
          throw new Error(
            `SG_ line at ${
              index + 1
            } does not follow DBC standard; should have eight pieces of text/numbers (or nine for multiplexed signals).`
          );
        }
        ...
        try {
          ...
          if (signalData.min >= signalData.max) {
            problems.push({
              severity: 'error',
              line: index + 1,
              description: `SG_ ${signalData.name} in BO_ ${currentBo.name} will not show correct data because minimum allowed value = ${signalData.min} and maximum allowed value = ${signalData.max}. Please ask the customer for a new .dbc file with correct min/max values if this errors pops up often.`,
            });
          }
          ...
          if (signalData.sourceUnit) {
            switch (signalData.sourceUnit.toLowerCase()) {
              ...
              case 'm':
                if (
                  signalData.label.includes('distance') ||
                  signalData.label.includes('odo')
                ) {
                  signalData.postfixMetric = 'km';
                  signalData.postfixImperial = 'mi';
                  signalData.factor /= 1000; // TODO, log postprocessing events like these
                  signalData.offset /= 1000;
                  signalData.min /= 1000;
                  signalData.max /= 1000;
                } else {
          ...
          problems.push({
            ...
            line: index + 1,
            description:
              'Can\'t parse multiplexer data from SG_ line, there should either be " M " or " m0 " where 0 can be any number. This will lead to incorrect data for this signal.',
          });
        }
      ...
      case 'VAL_': // VAL_ 123 signalWithValues 0 "Off" 1 "On" 255 "Ignore" 254 "This device is on fire" ;
        ...
        if (line.length % 2 !== 0) {
          problems.push({
            ...
            line: index + 1,
            description:
              'VAL_ line does not follow DBC standard; amount of text/numbers in the line should be an even number. States/values will be incorrect, but data is unaffected.',
          });
          return;
        }
        ...
        if (line.length < 7) {
          ...
          problems.push({
            ...
            line: index + 1,
            description:
              'VAL_ line only contains one state, nothing will break but it defeats the purpose of having states/values for this signal.',
          });
        ...
        const { valBoLink, valSgLink, states } = extractValData(line);
      ...
      case 'SIG_VALTYPE_': // SIG_VALTYPE_ 1024 DoubleSignal0 : 2;
        ...
        if (line.length !== 5) {
          throw new Error(
            `SIG_VALTYPE_ line at ${
              index + 1
            } does not follow DBC standard; should have a CAN ID, signal name and number.`
          );
        ...
        const { dataTypeBoLink, dataTypeSgLink, dataType } =
          extractDataTypeData(line, index + 1);
      ...
      case 'CM_': // CM_ [<BU_|BO_|SG_> [CAN-ID] [SignalName]] "<DescriptionText>";
        if (line.length < 4) {
          throw new Error(
            `CM_ line at ${
              index + 1
            } does not follow DBC standard; should have one of (BU_, BO_ or SG_) type, CAN ID, signal name and comment.`
          );
        ...
        const { commentBoLink, commentSgLink, comment } = extractCommentData(
          line,
          index + 1
        );
      ...
      default:
        debug(
          `Skipping non implementation line that starts with ${line}`,
          line
        );
    }
  });

  if (!_.isEmpty(currentBo)) boList.push(currentBo);
  ...
  valList.forEach((val) => {
    const bo = _.find(boList, { canId: val.valBoLink });
    if (!bo) {
      problems.push({
        severity: 'warning',
        line: val.lineInDbc,
        description: `VAL_ line could not be matched to BO_ because CAN ID ${val.valBoLink} can not be found in any message. Nothing will break, and if we add the correct values/states later there won't even be any data loss.`,
      });
      return;
    }
    const sg = _.find(bo.signals, { name: val.valSgLink });
    if (!sg) {
      problems.push({
        severity: 'warning',
        line: val.lineInDbc,
        description: `VAL_ line could not be matched to SG_ because there's no signal with the name ${val.valSgLink} in the DBC file. Nothing will break, but the customer might intend to add another signal to the DBC file, so they might complain that it's missing.`,
      });
      return;
    }
  ...
  dataTypeList.forEach((dataType) => {
    const bo = _.find(boList, { canId: dataType.dataTypeBoLink });
    if (!bo) {
      problems.push({
        severity: 'warning',
        line: dataType.lineInDbc,
        description: `SIG_VALTYPE_ line could not be matched to BO_ because CAN ID ${dataType.dataTypeBoLink} can not be found in any message. Nothing will break, but the customer might have intended to add another message to the DBC file, so they might complain that it's missing.`,
      });
      return;
    }
    const sg = _.find(bo.signals, { name: dataType.dataTypeSgLink });
    if (!sg) {
      problems.push({
        severity: 'warning',
        line: dataType.lineInDbc,
        description: `SIG_VALTYPE_ line could not be matched to SG_ because there's no signal with the name ${dataType.dataTypeSgLink} in the DBC file. Nothing will break, but the customer might have intended to add another signal to the DBC file, so they might complain that it's missing.`,
      });
      return;
    }
  ...
  commentList.forEach((comment) => {
    const bo = _.find(boList, { canId: comment.commentBoLink });
    if (!bo) {
      problems.push({
        severity: 'warning',
        line: comment.lineInDbc,
        description: `CM_ line could not be matched to BO_ because CAN ID ${comment.dataTypeBoLink} can not be found in any message. Nothing will break, but the customer might have intended to add another message to the DBC file, so they might complain that it's missing.`,
      });
      return;
    }
    const sg = _.find(bo.signals, { name: comment.commentSgLink });
    if (!sg) {
  ...
  problems.forEach((problem) => {
    ...
    const message = _.find(boList, { lineInDbc: problem.line });
    if (message) {
    ...
    boList.forEach((bo) => {
      bo.signals.forEach((sg) => {
        const signal = _.find(sg, { lineInDbc: problem.line });
        if (signal) {
  ...
  const result = { params: boList, problems };
